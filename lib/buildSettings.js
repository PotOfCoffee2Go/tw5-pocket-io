...
exports.buildSettings = function buildSettings(config) {
	...
	if (dirNames.indexOf('Home') > -1) {
		dirNames.splice(dirNames.indexOf('Home'), 1);
		dirNames.unshift('Home');
	} else {
		throw new Error(`Missing wiki '${wikisDir}/Home' that comes with the GitHub project'`);
	}
