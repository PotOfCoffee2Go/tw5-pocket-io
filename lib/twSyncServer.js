...
exports.TwSyncServer = function TwSyncServer(twi, $tw, host, port, resolveCaller) {
	var self = this;
	this.twi = twi;
	this.$tw = $tw;
	this.host = host;
	...
	this.hook = (server, nodeServer) => {
		...
		setTimeout(() => {
			showStdout();
			log(`'${self.$tw.boot.wikiPath}' webserver ${self.restarted}started - $tw instance ${self.twi}`);
