...
module.exports = function () {
	...
	program
	  ...
	  .option('-d, --downloadGist', 'Install files from a gist')
	  .option('-e, --empty', 'Install with only a single empty wiki');
	...
	// Insure config.js and security.js
	if (!fs.existsSync(configPath)) {
		if (!opts.downloadGist) {
			try {
			...
			// Default TW5-Node-Red configuration files from Gist
			const gist = 'https://gist.githubusercontent.com/PotOfCoffee2Go/' +
				'1ec090fdfaac70196305f97620dac08e/raw/';
			const files = [
				{ gist: 'config.js' , host:  configPath },
				{ gist: 'security.js', host: securityPath }
			];
			const copyFromGist = new FetchGist();
			copyFromGist.resources(gist, files);
		}
	}
	...
	if (config.pkg.poc2go && config.pkg.poc2go.install === 'tw5-node-red project') {
