...
const psystem = 'Pocket-io System.tid';
...
exports.verifyCodebase = function verifyCodebase() {
	...
	if (!fs.existsSync(`${cbaset}/${psystem}`)) {
		hog(`'${cbaset}/${psystem}' is required!`, 9);
		process.exit(5);
	}
}
...
exports.copyClientTiddlers = function copyClientTiddlers(wikisDir, dirNames) {
	dirNames.forEach(name => {
		...
		if (fs.existsSync(`${wikisDir}/${name}/tiddlywiki.info`)) {
			...
			fs.copyFileSync(`${cbaset}/${psystem}`,
				`${wikisDir}/${name}/tiddlers/${psystem}`);
