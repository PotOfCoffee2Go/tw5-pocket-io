...
exports.PocketIoServer = function PocketIoServer(config, serverSettings, isPublic) {
	...
	if (isPublic) {
		...
		this.domainName = config.domainName === 'http;//' + os.hostname
			? `${config.domainName}:${this.port}`
			: `${config.domainName}`;
	} else {
		this.host = '127.0.0.1';
		this.port = config.proxy.private.port;
		this.domainName = `http://localhost:':${this.port}`;
	}
	...
	this.addTarget = function addTarget(config, settings) {
		...
		const createLink = () => {
			if (this.domainName === 'http://' + os.hostname()) {
				return `${this.domainName}:${this.port}/${settings.name}`;
			} else {
				return `${this.domainName}/${settings.name}`;
			}
		}
		...
		if (config.proxy[accessType()].wikis.includes(settings.name) ||
			...
			settings.proxy.domainName = this.domainName,
			settings.proxy.link = createLink();
