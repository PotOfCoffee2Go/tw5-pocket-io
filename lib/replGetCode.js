...
function getCodeFromWiki($rt, wikiName, $tw, filter, verbose) {
	var byteCount = 0, hasErrors = false;
	hog(` Autoload - ${wikiName + ': ' + filter.replace(/\]\]/g, ']]')}`, 43);
	...
	hog(`  ${tiddlers.length} code tiddlers loaded - ${(byteCount/1024).toFixed(3)}K bytes.`, 43);
...
exports.replGetCodeFromWikis = function replGetCodeFromWikis($rt, serverSettings) {
	...
	codeList.forEach(codeFilter => {
		var { tiddlerCount, byteCount, hasErrors } = getCodeFromWiki($rt, codeFilter.wikiName, codeFilter.$tw, codeFilter.filter, false);
