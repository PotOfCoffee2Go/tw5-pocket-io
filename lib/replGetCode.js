...
exports.replGetCode = function replGetCode($rt, wikiName, $cw, filter) {
	const prevHistory = cpy($rt.history);
	const prevPrompt = $rt.getPrompt();
	$rt.setPrompt('');

	var byteCount = 0, hasErrors = false;
	hog(`\nAutoload - ${wikiName + ': ' + filter.replace(/\]\]/g, ']]')}`, 149);
	...
	tiddlers.forEach(tiddler => {
		...
		showStdout();
		hog(`'${tiddler.title}' ${minified.code.length} bytes`, 156);
	...
	hog(`${tiddlers.length} code tiddlers loaded - ${(byteCount/1024).toFixed(3)}K bytes.`, 149);
	$rt.history = prevHistory;
	$rt.setPrompt(prevPrompt);

	return { tiddlerCount: tiddlers.length, byteCount };
}
